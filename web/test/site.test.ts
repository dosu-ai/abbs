...
import { JSON_HEADERS, THREAD_ID, pageBody, serverInfoBody, threadBody } from "./helpers";
...
beforeAll(async () => {
  ...
  await seedWorkspace({ slug: "ws-one", name: "one-name", description: "agents building one" });
  await seedWorkspace({ slug: "ws-two", name: "two-name", description: "experiments and reports" });
  await seedWorkspace({ slug: "ws-down", baseUrl: "https://down.example", name: "down-name" });
  ...
  one
    ...
    .reply(200, serverInfoBody("one-name", { description: "agents building one" }), JSON_HEADERS)
    .persist();
  two
    .intercept({ path: "/v1/server" })
    .reply(200, serverInfoBody("two-name", { description: "experiments and reports" }), JSON_HEADERS)
    .persist();
  down.intercept({ path: "/v1/server" }).replyWithError(new Error("connect failed")).persist();
  down
    .intercept({ path: "/v1/threads", query: { limit: "50" } })
    .replyWithError(new Error("connect failed"))
    .persist();
  down.intercept({ path: "/v1/threads" }).replyWithError(new Error("connect failed")).persist();
  down
  ...
  const messages = [
    ...
    },
    {
      ...
      thread_id: THREAD_ID,
      author: "buildbot",
      content: "Conformance is green for reconnect from the last committed cursor.",
      deleted: false,
...
describe("api", () => {
  it("lists registry workspaces", async () => {
    ...
    expect(r.status).toBe(200);
    const body = (await r.json()) as { items: { slug: string; base_url: string }[] };
    expect(body.items.map((w) => w.slug).sort()).toEqual(["ws-down", "ws-one", "ws-two"]);
  });
  ...
  it("passes through protocol-shaped thread pages", async () => {
    ...
    const body = (await r.json()) as { items: { title: string }[]; next_page: string };
  ...
  it("returns workspace detail with live upstream state", async () => {
    ...
    expect(r.status).toBe(200);
    const body = (await r.json()) as { upstream: { state: string; error_code?: string } };
