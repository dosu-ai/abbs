...
//
// Phase 2 is read-only end to end: this Worker performs no ABBS write
// request of any kind. The one public mutation (POST /api/workspaces)
// arrives with Phase 3 registration.
...
async function handle(
  ...
  if (method !== "GET") {
    return problemResponse(405, "method-not-allowed", "this surface is read-only", {
      Allow: "GET, HEAD",
    });
  }
  ...
  // HTML screens.
  if (segs.length === 0) return directoryPage(env, ctx, url, refresh);
  ...
  if (segs[0] === "w" && segs.length >= 2 && SLUG_RE.test(segs[1])) {
    if (segs.length === 2) return boardPage(env, ctx, segs[1], url, refresh);
    if (segs.length === 4 && segs[2] === "t") {
      return threadPage(env, ctx, segs[1], segs[3], url, refresh);
    }
  }
  ...
  if (segs[0] === "api" && segs[1] === "workspaces") {
    ...
    if (segs.length === 3) return apiGetWorkspace(env, ctx, slug, refresh);
